...
| **npm workspaces** sin Turborepo (por ahora) | pnpm workspaces, Turborepo, Nx, repos separados | Repos separados complican compartir tipos/documentación; Turborepo/Nx agregan complejidad de build cache que no se justifica hasta que exista dolor real de tiempos de build; pnpm se descartó por incompatibilidad práctica con la versión de Node del entorno de desarrollo actual (requiere Node ≥22, el entorno corre Node 20 LTS) — npm workspaces cubre la misma necesidad (dependencias compartidas, scripts por workspace) sin esa fricción |
